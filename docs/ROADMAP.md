...
|-------|------|-----------|-----------|
| **Phase 1** | 2026-04-01 ~ 2026-04-14 (2주) | Notion 연동 완성 + API E2E 검증 | ✅ API Route에서 Notion 데이터 정상 반환, E2E 토큰 시나리오 통과, `npm run check-all` 통과 |
...
- [x] `.env.local` 파일 생성 및 필수 환경변수 세팅
...
- [x] `.env.example` 파일 작성 (팀 온보딩용, 실제 값 제외)
- [x] Vercel 프로젝트 생성 및 환경변수 등록

**[1-2] Notion 데이터베이스 스키마 확정**

- [x] Notion에 `Quotes` 데이터베이스 생성 (PRD 5절 속성 구조 기준)
...
- [x] 견적 항목 처리 방식 최종 결정: 페이지 본문 테이블 블록(옵션 A) vs 하위 데이터베이스(옵션 B)
  > ⚠️ 가정: 단순성을 위해 옵션 A(페이지 본문 테이블 블록)를 사용한다고 가정합니다. 최종 결정 후 항목 파싱 로직을 업데이트해야 합니다.
- [x] Notion Integration 생성 및 데이터베이스 연결 허용
- [x] 테스트용 견적서 페이지 3개 생성
...
- [x] `app/api/quote/[token]/route.ts` — 페이지 본문 블록 조회 및 파싱 로직 구현
  - `GET /v1/blocks/{page_id}/children` 호출
  - 테이블 블록 파싱하여 `QuoteItem[]`(항목명, 수량, 단가, 금액, 비고) 변환
  - `TODO` 주석으로 표시된 `rawItems` 매핑 로직 실제 구현으로 교체
- [x] Notion API 타임아웃 10초 설정: `AbortController` 활용
- [x] 토큰 형식 검증: 최소 16자 이상 체크 (현재 8자 → 16자로 상향)
- [x] 서버 시작 시 필수 환경변수 검증 로직 추가 (누락 시 명확한 오류 메시지)
- [x] `revalidate` 또는 `unstable_cache`를 활용한 60초 캐싱 전략 적용 (현재 `no-store` → 변경)
...
- [x] `npm run check-all` 통과 (lint + type-check + build)
...
- `npm run check-all` 오류 없음 (lint + type-check + build)
...
- [x] 비고(notes) 영역: 줄바꿈 처리 (`whitespace-pre-wrap`) 확인
- [x] PDF 다운로드 버튼: `window.print()` 호출 (클라이언트 컴포넌트 내 구현 확인)

**[2-2] 반응형 레이아웃 검증**

- [x] 모바일(360px): 공급자/클라이언트 정보 그리드 1컬럼 처리
- [x] 태블릿(768px): 2컬럼 레이아웃 전환 확인
- [x] 데스크탑(1280px): 최대 너비 컨테이너 적용 확인
- [x] 견적 항목 테이블 가로 스크롤: `overflow-x-auto` 처리 확인
...
- [x] Chrome / Safari / Firefox 세 브라우저에서 PDF 저장 테스트

**[2-4] 오류 페이지 UI 완성 (F010)**

- [x] `app/not-found.tsx` 커스텀 페이지 작성
  - 안내 문구: "유효하지 않은 견적서 링크입니다."
  - 서비스 제공자 연락처 표시 (`PROVIDER_EMAIL` 환경변수 활용)
- [x] 만료 오류 페이지: 만료일 표시 포함 (`expiry_date` 전달)
- [x] 서버 오류 페이지: 기술 용어 없는 한국어 안내 문구
- [x] 오류 유형별(404 / 410 / 500) 안내 메시지 사용자 친화적 문구 최종 검토

**[2-5] 로딩 UI 검토 (F011)**

- [x] `loading.tsx` Skeleton 항목 수를 실제 데이터 구조에 맞게 조정
- [x] Chrome DevTools 네트워크 조절(3G 시뮬레이션)에서 Skeleton 표시 확인
...
- [ ] `npm run check-all` 통과 (lint + type-check + build)
